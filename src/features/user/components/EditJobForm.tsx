import { useCategory, useEditJob } from "@/features/jobs/hooks/jobHooks";
import { EditJobSchema, type EditJobCredentials, type PostJobCredentials } from "@/features/jobs/schemas/postJobSchemas";
import { useDistricts, useRegencies } from "@/features/user/hooks/addressHooks";
...
const EditJobForm = ({ job, setIsDialogJobEditOpen }: { job: JobAssignment; setIsDialogJobEditOpen: (open: boolean) => void }) => {
  const defaultValues: Partial<PostJobCredentials> = {
    ...
    startAt: job.startAt ? format(new Date(job.startAt), "yyyy-MM-dd") : "",
    deadlineAt: job.deadlineAt ? format(new Date(job.deadlineAt), "yyyy-MM-dd") : "",
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<EditJobCredentials>({
    resolver: zodResolver(EditJobSchema) as Resolver<EditJobCredentials>,
    mode: "onChange",
    defaultValues,
  });
  ...
  const onSubmit = (data: EditJobCredentials) => {
    editJob(
      {
        jobId: job.id,
        payload: data,
